...
| | |
| --- | --- |
| **Do first** | 📋 **v1.21.0 is CUT but NOT PUBLISHED — that is the one open decision.** The bump is committed (`5cb0318`) and tagged **locally**; the zip is built. Nothing has been pushed to GitHub and **prod has not been touched, by instruction**. Publishing is `git push --follow-tags` + `gh release create` — see the Release row. ⚠ **Six suites have not run since the polish.js fold** (maneuvers, volleys, riders, concentration, playerdmg, savedmg); run them before publishing if the tag is meant to carry a battery claim. Everything else: **Phase 4 is DONE except D2**, and the refactor cost **zero features** (see Parity). |
| Repo | `main` @ `5cb0318`, **tag `v1.21.0` on it, LOCAL ONLY — not pushed.** Three commits this session: `666ec31` the release-zip fix · **`5cb0318` the v1.21.0 bump (tagged)** · the docs recut. Before them 2026-08-23: `46b4580` the d20-fold corrections · `17ac81c` the parity battery · **`0e2380a` D6** · `e143496` the census re-run · `393c9cb` the recut. Re-verified at session start: gate exit 0, `verify-settings` CLEAN. |
| Release | ⚠ **v1.21.0 CUT, TAGGED LOCALLY, NOT PUBLISHED — prod still runs v1.20.0 and pre-refactor code, deliberately.** `module.json` reads **1.21.0** and its manifest `download` points at the **v1.21.0** zip; both fields moved in one commit, which is the fix for the v1.20.0 class (that bump moved `version`, left `download` on v1.19.0, and it was caught at release). `dist/fvtt-mod-battleflow.zip` is built: **30 entries, 208,444 bytes**, forward slashes verified, all 99 relative imports resolving inside the archive. ⚠ **To publish:** `git push --follow-tags`, then `gh release create v1.21.0` with **BOTH assets** — the zip *and* a bare `module.json`. ⚠⚠ **The version-collision trap is only half closed:** HEAD says 1.21.0 but the **sandbox still carries the 1.20.0 `module.json`**, so redeploy it before trusting what the sandbox reports. |
...
| Sandbox | ⚠ **HEADLESS, LEFT RUNNING** (world active, 0 users) carrying **HEAD's `scripts/` byte-identical** but ⚠ **a STALE `module.json` (1.20.0 vs HEAD's 1.21.0)** — redeploy before any version-sensitive check — `status` first, `stop` if not testing. `node <mcp>/scripts/local-foundry.mjs start/stop/status/restart`. Never the Electron app for suites (dataPath lock). **Verify status at session start.** |
...
⚠ **User call, 2026-08-23 — feature scoping moved from #2 to the BOTTOM.** It sat at #2 only
because it had been named as the next step; that is no longer the standing intent. **The
standing directive therefore runs unbroken** (correctness and architecture only, no feature
work considered): finish the refactor, *then* scope. The surveyed features stay what they
always were — design pressure on the seams, not scheduled work.

| # | Work | Why this position |
| --- | --- | --- |
| 1 | ⚠ **Publish v1.21.0, or deliberately decide not to** | **The only thing genuinely owed, and now half done.** The cut is made: both `module.json` fields bumped in one commit, tagged `v1.21.0` locally, zip built and proven self-contained. What is left is outward-facing and was withheld by instruction — `git push --follow-tags`, then `gh release create v1.21.0` with the zip **and** a bare `module.json`. ⚠ **Decide first whether the tag needs a full battery behind it:** six suites have not run since the polish.js fold. Until this happens, prod runs pre-refactor code and the sandbox's own `module.json` still says 1.20.0. |
| 2 | **Phase 3 — registries unified** | **The only untouched phase**, and the last one that is a refactor rather than a decision. Its final bullet is now the live one: *"the rate of new kinds per phase is observable — that rate is the AC5e-adoption signal, and right now nobody could tell you what it is."* D8 says a new kind just arrived. One of its four bullets (registry integrity in the gate) is already done but unticked. |
| 3 | **D2 — `hold.js` onto the moment spine** | The last Phase 4 item and the highest-risk thing in PLAN.md. ⚠ **D6 made it materially easier**: the hold's views and clocks now sit in hold.js beside its flag, so adopting the spine's primitives is a local change rather than a cross-file one. Still ships **alone, behind its own walk**. ⚠ Its clock must **not** be unified — the hold's is owned by the continuing client, every other by the elect. |
| 4 | **The §4.1 relay** — three folds, three envelope keys, one shape | Removes two `createChatMessage` registrations from the pinned hook order. ⚠ **hold's folder has a different OWNER** (the continuing client; the other two are the elect), exactly like its clock. Unify the envelope, keep ownership pluggable. |
| 5 | **Feature scoping — LAST, and D8 is the frame** | ⚠ **Moved here by user call 2026-08-23; do not promote it back without one.** When it is reached: the three surveyed d20 features (Heroic Inspiration, Tactical Mind, Bardic Inspiration) are **one kind**, and the module already ships a member of it — Precision Attack. Scope them together, not one at a time (ARCHITECTURE §10 **D8**). ⚠ **The real new work is the SAVE side**: `hitsAmong` folds verdicts for attacks and has no equivalent for saves, so a rerolled or boosted save has nowhere to land. ⚠ And a reroll can turn a hit **into** a miss, which breaks `hitsAmong`'s documented "the sets are disjoint" precedence argument. Neither the offer nor the popup is the hard part. |

...
| Repo | `main`, **clean**, and **FOUR COMMITS AHEAD OF THE v1.23.1 TAG** — D10's dispatch check, D11's coverage ledger, `smoke-d20-folds` §3, and the docs. ✅ **BATTERY GREEN OVER ALL OF IT** (19m59s, settings CLEAN, world rolled back). Not pushed, not released. |
...
| Never fired | Verdict, 2026-08-23 |
| --- | --- |
| `updateCombat` · `deleteCombat` (hold.js) | ⚠ **THE REAL GAP, AND THE MOST USEFUL THING THIS REPORT HAS FOUND: no suite in the battery ever creates a COMBAT.** Verified by grep — nothing calls `Combat.create`, `rollInitiative` or `toggleCombat` anywhere in `tools/`. The hold's **per-turn clears are table-facing and completely unexercised.** This is the next test worth writing. |
| `createMeasuredTemplate` · `updateMeasuredTemplate` (saves.js) | ⚠ **Known, measured, and BURIED IN A SOURCE COMMENT until this report dragged it out** — `smoke-saves` §8 says *"the CRUD hooks measurably never fire on this page"* and routes around it by nudging the card's render. The template-adoption feature is table-proven (the Shatter and Moonbeam reports), so **what is untested is the hook path, not the feature.** ⚠ Worth knowing before trusting any placeable-document hook in a suite. |
| `dnd5e.rollToolCheck` (d20-folds.js) | Already a NAMED SKIP in `smoke-d20-folds` §4 — the fixture fighter carries no tool. **Cheapest line here to close:** grant one, the way §3's Longsword was granted. |
| `renderActivityUsageDialog` (polish.js) | **Structural, and correct.** Every suite passes `configure: false` precisely so no dialog renders. Covering it means a suite that deliberately opens one. |
| `renderSettingsConfig` (settings.js) | No suite opens the settings dialog. Pure UI polish on the config form; the cheapest thing on this list to leave alone. |

---

## ▶ What is NOT yet done

**2026-08-23. The tree carries four commits the tag does not.** Nothing here is blocking.

| Owed | Why it matters |
| --- | --- |
| **A release** | v1.23.1 is the tag; four commits sit above it. `node tools/bump-version.mjs minor` then `build-release.ps1` (which runs the gate itself, with no skip flag). |
| **Prod and the sandbox** | Both carry v1.23.1. ⚠ **Nothing in this batch changes shipped behaviour — `scripts/` is untouched.** It is all `tools/` and docs, so a deploy buys nothing but the version string. |
| **The combat-hook test** | The one genuine gap the triage found. See the table above. |
...
- ⚠ **Release notes are HAND-WRITTEN into `dist/RELEASE-NOTES.md`, never `NOTES.md`** — that is
  ...
  not the practice.
- ⚠ **THE NO-OP HANG-UP — found 2026-08-23, recorded rather than repaired.** `f.disconnect` **does
  not exist on `Foundry`; the method is `dispose()`.** Every suite in this tree ends with
  `await f.disconnect?.()`, and the optional chain has been swallowing it since the harness was
  written — the hang-up ceremony is decorative and the session is really torn down by process
  exit. **This is D11's own failure class living inside the test tooling**: a call that reads
  correctly, does nothing, and reports nothing. The harness now defines `f.disconnect` so it does
  exactly one useful thing (dumping the hook ledger); ⚠ **it still does not dispose.** Making it
  really dispose changes every suite's teardown — a `dispose()` that hangs would turn a green run
  into a watchdog abort — so it wants its own pass and its own battery.

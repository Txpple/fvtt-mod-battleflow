...
| | |
| --- | --- |
| **Do first** | 🔍 **RESUME THE v1.20.0 WALK** (the table below carries live statuses). Walked so far: 1–3 ✅ and 5's Gren half ✅ (user blanket, verbatim: *"when i do all the volleys with gren, works fine"*), 7 🟡 (*"on the self use showing usage, great"* + one timing finding). **Open: 4, 6, 8, 9, 10** plus the Salyth MM re-test under (bb). **Five findings (bb)–(ff) are diagnosed and designed but NOT BUILT — no "go" yet; (ff) is the user's own structural call.** (cc) and (dd) are scope-CONFIRMED by directive (2026-08-21: *"put in the fix for second wind and that pattern into the next test phase"*; *"adv/disav on attack roll rays is RAW, so we have to work with it"*). On "go": the fix-pass list at the end of the walk-1 section below. |
...
| Release | ⚠ **NOT RELEASED.** v1.19.0 remains the latest release and prod runs it. The v1.20.0 release steps (after the walk): tag → GitHub release (zip built by explicit ZipArchive entry names — **22 entries now**: volleys.js + resources.js joined) → prod deploy → ~~the prod SR graft~~ **STRUCK by (ff)** (the lookup list ships in code — no prod content writes needed; `fix-scorching-ray.mjs` demotes to optional hygiene) → `BF_TARGET=prod verify-settings` (new keys — `volleyList` joins them under (ff) — ride module defaults; CLEAN expected) → bounce by down-by-user/wake-by-connect → `probe-registered-version` reads 1.20.0. |
...
| --- | --- | --- |
| (bb) | *"as salyth, they were inconsistent and esp the scorching rays i never got the popup as to where to allocate"* — setup fact, from the follow-up: *"i rolled everything logged in as Matt the DM, no other clients on"* | **SR: solved on paper — Salyth's copy carries NO count.** Detection is structural (`target.affects.count` evaluating 2+); the graft ran (thirteenth session) when only Gren + BF Test Shielder carried SR, so Salyth's copy is bare = never a volley = fully native — the popup was impossible on ANY window. Re-run `tools/fix-scorching-ray.mjs` (idempotent, sweeps every carrier). **MM: TWO candidates left** — the one-client fact kills routing, and the reference `autoDamage: 'all'` kills the `modeAllows` type gate ([shared.js:46](scripts/shared.js); both checked this session): (1) the TARGETLESS QUIET — no targets selected at cast time stays native by design, and reads "inconsistent" if targets were sometimes picked after the cast; (2) Salyth's MM is a DDB-imported copy whose count field was stripped — the exact damage class Gren's SR import proved exists. The fix-pass probe reads Salyth's actual MM item and settles it; either fix is cheap (target-first habit; the count hole itself is now (ff)'s to close — the lookup list makes Salyth's SR volley with NO graft) |
...
| (ff) | *"for scorching ray and eldritch blast, and any other spells possibly like that, we need to make a lookup list for those types? clearly we just cant rely on stamps on the world canvas"* | **AGREED — graft-as-correctness is RETIRED.** Per-copy stamps rot: every new character, DDB import or late-added spell arrives bare, and Salyth proved it within a day of the graft. Design: a WORLD SETTING **`volleyList`** in the house list idiom (the blockList/riderList precedent, name-keyed, user-editable at the table): default `Scorching Ray: 1 + @item.level; Eldritch Blast: 1 + floor((@details.level + 1) / 6)` — SEMICOLON-separated because count formulas may carry commas (min/max). Detection order: the item's OWN count first (content wins — MM ships it, existing grafts stay harmless and equal), the list by item name as fallback, neither → native. Same evaluator (`volleyCount`'s rollData already carries `@item.level` = cast level, `@scaling`, and actor data — so `@details.level` drives EB's beams-by-character-level; an NPC copy that scales differently can carry its own count on the item, and content wins). Homebrew multi-projectile spells become a settings edit, zero releases. `fix-scorching-ray.mjs` demotes to optional content hygiene; the prod graft release step is struck from the Release row. Reference table + verify-settings grow the key. Suite: smoke-volleys grows a list-fallback section (a bare-count fixture volleys via the list; an emptied list goes native) |

**The fix-pass list, armed for "go" (one battery-green pass, the standing cycle):**
1. Build (ff) FIRST — the `volleyList` world setting (seeds SR + EB; content's own count
   wins; reference table + verify-settings grow the key; the graft becomes optional
   hygiene). Probe Salyth's MM count data while connected (the (bb) decider) + confirm
   EB's native data is bare (seed sanity).
2. Build (cc) the deferred flash, (dd) per-ray adv/dis, (ee) the target decoration.
3. Amend design.md Phase 1.7 + the resources subsection with all three (the §4.3 mandate:
   the map stays with the machines) — and note the straight-roll default's correction there.
4. Pins: smoke-volleys grows (dd) + (ee) sections, smoke-resources grows the (cc)

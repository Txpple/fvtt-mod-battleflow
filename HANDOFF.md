...
| Repo | `main`, clean, **NOT pushed** — the foundation pass sits above `origin/main`: stages 1–2, stage 3, D8, the version bump, and the docs. ⚠ **The count is deliberately not written here** — it went stale twice in one afternoon, once because writing it down was itself a commit. `git log --oneline origin/main..HEAD`. |
...
| Sandbox | ⚠ **HEADLESS.** `node <mcp>/scripts/local-foundry.mjs start|stop|status|restart`; deploy with `node <mcp>/scripts/deploy-house-module.mjs fvtt-mod-battleflow --local` (never without `--local`). Never the Electron app for suites (dataPath lock). **Stopped, deployed and restarted for the D8 battery**, so its `scripts/` are HEAD byte-identical. ⚠ **LEFT RUNNING** (world active, 0 users, pid 32180) — `status` first, `stop` if you are not testing. ⚠ **Its `module.json` still says 1.21.0**, because the bump happened after the deploy: harmless (no suite reads the version, and scripts are what the suites exercise), but a `deploy --local` will report that one file as changed. |
...
And the commits, which are not pushed either:

...
committed and pushed** (main == origin/main). ✅ **RELEASED as v1.35.0 on 2026-09-10, then v1.35.1 the same day, then v1.36.0 the same night (the stale Shield fixed, a feature's area swept, the Spent Areas list), then v1.37.0 on 2026-09-11 (Tactical Mind's refund ask), then v1.38.0 later that day (the moment events — what the module publishes at a resolve, for FX Studio v0.3.0 to play)** — the reroll fixes (four dice under advantage, the dice animating, the popups closing at the click), the durability review, the cantrip window, the window tidies (the user:

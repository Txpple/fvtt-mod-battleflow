...
| [geometry.js](scripts/geometry.js) | EDGE, not in the layer: `tokensInTemplates` (and its own `templateShape`) — they need canvas/CONFIG/PIXI; `tokenOfActor`, `tokenForUuid`, `feetOf`, `nearestFeet` — token distance in FEET, the readers the reminder gate grew and the damage service shares (the 5-foot automatic crit, 2026-09-02) |

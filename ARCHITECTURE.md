...
### The driver table — machine × moment × driver × no-GM (Stage 5 of the machine-tier pass, 2026-09-05)

**One question, `drivesMomentFor(subjectUuid)` in core.js** — the active GM, and with no GM the
subject's own player; identical to `isActiveGM()` whenever a GM is on. The three machines that
still carried their own copy of that body (mastery's `drivesMastery`, the hold's
`drivesSpellDamage`, auto-apply's `drivesPayouts`) keep only their SUBJECT resolution now; the
answer is core's. **"GM-only"** below means `isActiveGM()`, and the moment simply waits for a GM
— every such row says why. `canApplyTo(actor)` guards each world-write inside a driven moment
and `whisperNoGM()` names what did not land; `smoke-nogm` asserts the flow-elect rows (§runs,
§chip, §told, §conc, §spent, §cast) and §rejoin's rule: a GM rejoining re-pays nothing.
...
| **cast (ruling 4)** | the cast slice — a utility cast's effects, a heal's number | **the caster** | the caster's own sheet and any PC the caster's player owns land; a monster is whispered, the card marked asked-and-answered, and a GM rejoining re-pays nothing (§cast). ⚠ **The one behaviour change of the pass:** until 2026-09-05 this slice waited for a GM and applied LATE on rejoin |
| concentration | the ask, the fold, the break | the concentrator | everything — a PC owns their own sheet (§conc) |
| saves | the fold, the verdict (on the card — the v1.19.0 public line retired 2026-09-18), the consequences, the areas | the demand's caster (`sourceUuid`) | verdicts are public and true; consequences land on PC savers and are whispered for monsters |
...
supersede in topple.js), and `smoke-nogm` §rejoin exists to keep it that way: a GM reconnecting
mid-flight must re-pay nothing.

**Scope: every machine that has a subject** — mastery, damage/effect riders, **concentration,
saves and the hold** (v1.27.2 completed the sweep), **and the cast slice** (ruling 4, 2026-09-05 —
the last one still waiting for a GM). One helper answers it everywhere, `drivesMomentFor(subjectUuid)`,
and since Stage 5 no machine keeps a copy of its body; each supplies the subject its flag already
carries (§3 has the full driver table):
...
chained roll (`system.origin` — `originatingMessage` until 6.0) answers the card it chains to or nothing; a bare sheet roll
...
through. The flag is read or written outside `saves/` only through those.
...
✅ **THE RESUME FLOOR IS ONE PRIMITIVE (the machine-tier pass, Stage 3, 2026-09-05).** The idiom
— one in-flight set per file, the same driver registered on `createChatMessage`,
`updateChatMessage` and `dnd5e.renderChatMessage` (arrival, the write that releases a claim, the
reload), a claim on the card so history is inert — was copied about fifteen times, and the newest
copy (the damage shields) judged world state on a re-render without a claim. A machine declares
its moment now — `registerResumable(flagKey, { pending(flag, message, cause), drives(flag,
message), drive(message) })` in [ui.js](scripts/ui.js) — and the spine holds the three
registrations once, the latch keyed `${flagKey}|${messageId}`, and awaits the drive under it.
`cause` is `create` | `update` | `render`, because an arrival and a resume are different questions
(the appliers resume only an ex-claimed roll; the shields judge a released roll once). **The drive
does no DOM work**, so card-row order is untouched: the machine's own render hook keeps drawing
its row where it did. Ten resumables ride it: the cast slice and its healing, Bait and Switch, the
hit menu's effects, follow-ups and sweep, Cunning Strike's follow-ups, the damage shields, the
attack payouts and the spell damage. ⚠ Two are **`flagless`** — the payouts and the shields judge
an ARRIVAL that carries no module flag (the attack's damage roll is the system's own message), so
for them the key is a name and the machine reads what it needs off the message; everything else is
keyed on the flag it is a view of. The claims themselves (`effectsApplied`, `resolving`, `judged`,
the receipt) stay the machines', through the serializer, unchanged. The saves, concentration and
mastery resume checks stay inside their own render hooks (their rows and their resumes share a
registration; the Stage 0 snapshot is what would prove a later move order-free).
...
| [decide/emanations.js](scripts/decide/emanations.js) | `reachAdmits` (who an aura reaches, by disposition — helpful: allies and neutrals; harmful: enemies), `emanationRange` (the activity's size, else the row's content formula over the source's roll data — `@scale.paladin.aura`), `resolveChanges` / `resolveFormula` / `foldArithmetic` (the pack's effect with the SOURCE's numbers read in), `triggerDue` (once per turn in combat), `liveScenes` / `appliesOnScene` (the active scene and every scene a connected player views, a GM's while no player is connected — 2026-09-23; the cross-scene bleed, 2026-09-04), `emanationGroup` / `groupMembers` (one aura across every scene it stands on, one copy per creature), `memberEffectData` (the effect a member receives, fingerprinted with its aura for the floor) |
...
| [decide/card.js](scripts/decide/card.js) | THE CARD SEAM (the dnd5e 6.0 pass, phase 1, 2026-09-15): `CARD`, `cardKind`, `isCard`, `rollKindOf` / `rollKindInData`, `subKindOf` (a death or concentration save is a `save` told apart by `system.type`); `targetsOf` (6.0's token-keyed `system.targets` in the house shape — `uuid` is the target ACTOR, one row per actor, the token beside it), `describeTarget`, `TARGETS_KEY`; `originIdOf` / `originIdInData` / `originData` / `ORIGIN_KEY` (`system.origin` — the key the platform's registry indexes; every roll the module drives writes it); `activityRefOf` / `activityUuidOf` / `activityTypeOf`, `itemRefOf` / `itemUuidOf` / `itemNameOf`; `abilityOf`, `castLevelOn` (`system.level`), `scalingOf`, `concentrationIdOf` — what kind of card a message is, whose, from which, read off `type` and `system.*` ONLY; the ~90 `flags.dnd5e` reads that failed closed at 6.0 go through here, and a reader joins WITH its customer (phase 2 adds the mastery, the attack mode, `onSave` and `resisted` beside the files that read them) |
| [geometry.js](scripts/geometry.js) | EDGE, not in the layer: `tokensInRegions` — the platform's own containment (`TokenDocument#testInsideRegion` against the region's polygon tree, from the document's committed source position, no canvas; the 6.0 pass, phase 3 — `tokensInTemplates` and its shape ladder went with the template); `tokenOfActor`, `tokenForUuid`, `feetOf`, `nearestFeet` — token distance in FEET, the readers the reminder gate grew and the damage service shares (the 5-foot automatic crit, 2026-09-02) |
| [lookup.js](scripts/lookup.js) | SPINE, not in the layer (the machine-tier pass, Stage 1, 2026-09-05): `lower`, `itemNamed`, `featureNamed`, `activityNamed`, `activityOfType` (and their own `sameName`) — the sheet readers nine machines each carried; `resolveUuid` — the `fromUuidSync` guard that thirty sites wrote inline (a throw on an unloaded pack or a dead document is "not here"); `resolveDie` — a die formula resolved on THIS actor's roll data, an `@` left standing refused (five copies of the superiority-die idiom). `cardItem` / `cardActivity` (2026-09-22) — a CARD's item or activity: the live document while it stands, else the card's own snapshot of an item the use DELETED (dnd5e spends before it posts: a used-up potion or scroll is gone before its card exists); `tools/check-card-reads.mjs` holds every such read to them, a deliberate live read saying why. Imports only the card seam (decide/card.js — downward), owns nothing. `core.js` and `shared.js` keep their own uuid guard: the leaf and this file's own layer. Its two pure companions: `tableIndex(table, keyOf)` in decide/registry.js (a name-keyed table's closed name set and its row-by-name — the six `*_NAMES` sets derive through it now, **the keys unchanged**) and `listedNames(entries)` in settings.js (a list's kinds as a lower-cased set, seven copies before) |
| [surfaces.js](scripts/surfaces.js) | CORE, a second leaf beside core.js (the 6.0 pass, 2026-09-15): `SURFACES` — every HTML anchor the module reads off the PLATFORM's markup (core's `.message-content`, `[data-message-id]`, `.form-group`; dnd5e's damage tray, the usage card's SUMMARY block (`cardSummary` — where a chained roll's rows draw, ui.js `cardRow`; the card's BUTTONS are data since 6.0, filtered at birth in polish.js), the roll dialog's parts, default and roll buttons, the usage dialog's footer) and `SURFACE_SOURCES`, where each is authored. **No selector string for a platform element lives anywhere else** — `tools/check-surfaces.mjs` fails the build on one, and on a dnd5e anchor that no longer appears in the verified version's shipped templates (`dnd5e-surfaces.json`, generated and pinned like the hook artifact). NOTES §2 *the 6.0 pass* §3b: the card's DATA is the contract, its HTML is not |
...
**A machine may be a DIRECTORY** (2026-09-05, the machine-tier pass — ruling 3; `saves/` is the
first). When one flag's lifecycle outgrows a file, the directory is the unit the dependency rule
tests: its parts may import each other (their cycles are the tree's kind — hoisted `function`
declarations called at hook time), `index.js` is its only public face and fixes the registration
order of its parts by its import list, and the entry imports the index. ⚠ **That import list is
load-bearing, and its comment says so** — ESM evaluates a cycle's first-listed member LAST, which
is why `saves/index.js` lists `verdict.js` before `ask.js` and the snapshot is what proves the
order. [tools/check-layers.mjs](tools/check-layers.mjs) holds the rule as `GROUPS`: an edge inside
a group is legal, an edge from outside to any part but the face fails with *"import the index"*,
and the face is judged by the ordinary depth rule. The tier table lists `saves/` as one machine;
`EXPECTED_SOURCE_FILES` counts its parts file by file. **`hold/` took the same rule later the same
day, the second customer** (`hold.js` — one flag, 1,564 lines — cut into nine parts by moment:
`lookup`, `clock`, `trigger`, `spell-hold`, `answer`, `continue`, `spell-damage`, `views`, and the
index, which re-exports the machine's ONE outside name, `stampHoldIfInterrupted`). Its parts are a
DAG — no cycle, so no first-listed-evaluates-LAST trick, and a later part imports an earlier one
statically. The index's import list reproduced the old registration order with ONE line moved:
`clock.js` is imported by the triggers, so its delete sweep registers first (order-neutral — every
delete handler in the tree sweeps its own key; the snapshot commit says so). Two bare imports at
the head of the index (`ui.js`, `effect-riders.js`) keep the spine's bar registered above the hold
row, as hold.js's own import list did. The saves cut and this one measured about a session each.

**Split by MOMENT, not by phase** (the same day, Stages 4a and 4b): `maneuvers.js` became
`precision.js`, `riposte.js`, `hew.js`, `bash-offer.js` and `command.js` in its one entry slot,
the sheet readers going to `lookup.js` and the rules text to `decide/registry.js`; `mastery.js`
kept the payouts, the ask, the notices and the Cleave arm while `topple.js` took the `topple`
flag's lifecycle off the card mastery posts (the card is the bus, the emanations → saves
precedent) and `chip-spend.js` took the spend and the two tidies. Each split's cost was measured
by the snapshot: shared plumbing registrations became each feature's own (3 → 11 lines in the
maneuvers slot, 3 → 7 in mastery's), every relative order that renders on a shared card kept.
...
in the hold (`hold/views.js` since the directory cut), beside the flag they are a view of.
**`auto-damage.js` is the only file that imports the hold at all** (`hold/index.js`), and that is
hold's own feature API (`stampHoldIfInterrupted`) on
...
| `dnd5e.renderChatMessage` | every card row this module draws — fires for **every** message subtype. ⚠ Since 6.0 a save or check chained to a usage card renders HIDDEN and as a SUMMARY inside the usage card (client setting `chatCardSummary`): a row that can land on such a roll registers through ui.js `cardRow`, which draws it on the shown card or inside `.card-summary[data-message-id]` when the usage card renders — one drawer, two hosts, never both (the 6.0 pass, phase 4). The saves card's per-target line goes further (2026-09-18): a resolved target whose roll is summarized in the card gets the verdict's tail written into the platform's own row (`SURFACES.summaryRow`, `verdictTail`) and no line — one row per creature |
| `dnd5e.preCreateUsageMessage` (the 6.0 pass, phase 4) | the usage card's buttons are DATA (`system.buttons[]`): polish.js drops every row but `refundResource` before the card exists — a button not in the data is on no client's card; the handlers underneath survive |
| `updateChatMessage` → the summary nudge (ui.js, the 6.0 pass, phase 4) | the platform re-renders a summarized roll's origin on create, delete and `system` change only; when this module's flags move on a summarized roll (a fold answered), ui.js calls `ui.chat.updateMessage(origin)` so the summary re-draws |
| `Actor5e#applyDamage` | the resistance math (N1 — never reimplemented) |
| `Actor5e#rollSavingThrow` / `rollConcentration` | real saves (N1) |
| the message registry (`system.origin`, `getAssociatedRolls` — `originatingMessage` until 6.0; every roll the module drives writes `system.origin`, decide/card.js `originData`) | chain resolution — **we ride the system's registry, never a parallel one** |
...
| `dnd5e.rollSkill` / `dnd5e.rollToolCheck` — their `data` carries `skill` / `tool` (5.3.3, unchanged at 6.0.1) | the scoped folds (Ambush on Stealth; Tactical Assessment on History, Investigation, Insight) read the skill off the hook's own data |
...
| **Regions (core, 2026-09-03)** — `RegionDocument.createTokenEmanation`, `Region.attachment.token`, `region.tokens`; the events `tokenEnter` / `tokenExit` / `tokenTurnEnd` / `behaviorActivated` / `behaviorDeactivated` delivered to a behaviour type registered through `CONFIG.RegionBehavior.dataModels` (the way dnd5e registers difficult terrain; the subtype declared in `module.json` `documentTypes`); the hooks `createRegion` / `deleteRegion` / `updateRegion` / `updateToken` | emanations.js: the aura's area is the platform's — shape, attachment, membership and events — and the module only decides what it hands out. ⚠ Since dnd5e 6.0 an activity's area IS a Region (`TemplatePlacement.fromActivity`, stamped `flags.dnd5e.activity` — the tie every path keys on — `item`, `origin` = the usage TOKEN, `spellLevel`, `dimensions`); `Scene#templates` is deprecated at Foundry 14 (a drawn template is a Region wearing `flags.core.MeasuredTemplate`) and the `*MeasuredTemplate` hooks never dispatch — saves/areas.js rides `createRegion` / `updateRegion` and sweeps Regions; a placed region is NO concentration dependent, the `deleteActiveEffect` sweep ends a duration area — the saves machine's for a cast with a demand, emanations.js `endConcentrationAreas` for every other concentration cast (the areas the caster's client tied to the concentration effect at the cast, `areas` on the effect written at `dnd5e.postUseActivity`; the activity match only for an untied area nothing else can claim — Fog Cloud, 2026-09-19). ⚠ `region.tokens` is filled by `RegionDocument#updateTokens` with `noHook: true` — a just-raised ring's membership lands silently, so every membership floor tests geometry itself (`tokensInRegions`). The platform's own `dnd5e.*` behaviours are refused on any region this module adopts (`preCreateRegionBehavior`, ruling 3 of the pass) |
...
understood and deliberately unpaid — two edges since the machine-tier pass (four when this was
written), each pinned with the condition that would repay it. D10 was a failure class with no rule against it, and now has one: a hook name the system
...
| # | Debt | Violates | Evidence |
| --- | --- | --- | --- |
| **D9** | → **BACKLOG, NOT OWED ([BACKLOG.md](BACKLOG.md)) — five of seven repaid (two of them by the machine-tier pass, 2026-09-05), two parked by decision with self-expiring pins.** The full argument is kept here as the permanent record. ⚠ **PARTLY REPAID, PARTLY PARKED (2026-08-23).** **Seven pinned edges were a machine importing another machine** — the exact shape D1 repaid for `hold.js`, grown back one convenience at a time in files D1 never touched. ⚠ **This row exists because the ENFORCEMENT PASS made the graph countable**, and it was deliberately never a promise to repay all seven. **Five are repaid; two stand, each reasoned.** ✅ **(a) CLOSED — `dramaticVerdictPause` moved `concentration.js` → `ui.js`.** A presentation-timing primitive with two outside customers (`saves.js`, `mastery.js`), which is what made it a service rather than a favour; the spine owns HOW a moment is presented (D6's generalisation). ⚠ **It was NOT order-neutral and the reasoning that said it would be was wrong** — dropping `mastery.js → concentration.js` moved concentration.js LATER on five hooks (`renderChatMessage`, `create`/`update`/`deleteChatMessage`, `damageActor`), because mastery's import had been pulling concentration's evaluation ahead of the entry order. All twelve hook-order assertions still pass, and the move is unobservable for a specific reason worth keeping: **concentration's row renders only on its own ask card**, and every handler whose order changed reads a DISJOINT flag namespace (`d20fold` vs `concentration`). ⚠ The one genuinely contended pair — concentration before `saves.js` on `createChatMessage`, where a save roll could be folded by either — **is preserved in both orders.** ✅ **(b) CLOSED — `combatStamp` moved `mastery.js` → `core.js`**, beside `inRunningCombat` in the §3 "who/when" family. Empirically order-neutral: the printed evaluation order is byte-identical before and after, because the entry already evaluated mastery before maneuvers. ✅ **(c) CLOSED 2026-09-05 (the machine-tier pass, Stage 4a).** It was `saves.js` → `maneuvers.js` (interpose: `foldEntryFor`/`equippedShield`/`RULE_TEXT`) — genuinely cross-feature: a save's verdict opens a maneuver's choice. The IMPORT half is repaid by moving what both sides read DOWNWARD: the sheet readers to `lookup.js`, the rules text to `decide/registry.js` beside `MASTERY_RULES`; the pin came out under the stale-pin rule, and `maneuvers.js` itself is five files. The **save-choice registry beside `SAVE_FOLDS`** the row also named stays feature-shaped work for the third choice kind (the D8 lesson) — the choices live in `saves/choices.js` on the `saves` flag, no new key. ✅ **(d) CLOSED 2026-09-05 (the machine-tier pass, Stage 3b — ruling 2: now, not on a third customer; the abilities sweep is the third). The spine owns withhold-and-resume: `registerWithhold` / `registerWithheld` / `withholds` / `resumeWithheld` in ui.js; both lazy imports are gone and the two pins came out of `ALLOW` under the stale-pin rule.** It was `saves.js` ↔ `d20-folds.js`, a TWO-WAY cycle — `offerFoldOnSave` out, `foldSaveAnswer` back: saves.js withholds a verdict while an offer is live and d20-folds hands it back. ⚠ **Nobody knew this cycle existed until the check printed it**, one day after a by-hand architecture review of the same tree missed it. Not a defect — the protocol is correct and fails open — but **withhold-and-resume is moment lifecycle, and the spine owns moment lifecycle (§5)**. The third machine pair to grow a two-way edge is the argument for a spine primitive. **Disposition: was OPEN for the next moment that needs to withhold; repaid ahead of it on the user's ruling.** ⚠ **(e) `saves.js` → `receipts.js` (`revertTarget`, lazy)** is the classification question this row leaves standing: `receipts.js` is filed as a MACHINE because `revertTarget` has exactly one importer. **A second importer makes it a service — reclassify then, not now.** ⚠ **(f) GROWN SINCE THIS ROW WAS WRITTEN (found 2026-09-03 by re-reading the printed table, not the row): 8 pairs became 11.** `auto-damage.js → sneak.js` and `auto-damage.js → clock-riders.js` joined `auto-damage.js → mastery.js` — three lazy up-edges of ONE declared shape, the damage OFFER (a service) importing a machine to paint that machine's content on the popup (the pin's own words: *"the same shape as its lazy edge to mastery.js"*). By this row's own rule the third instance is what proves a seam — an offer-contributions list the machines declare into and the service walks, `ATTACK_FOLDS`-style. ✅ **REPAID 2026-09-04, on its trigger** — the hit menu landed on the damage offer and built the seam with it (`registerOfferPart` in auto-damage.js, §7 *The offer's contributions*); the three pins came out and the table read 8 pairs again. The two clock residues found beside it stay parked — [BACKLOG.md](BACKLOG.md) *Two clock residues*. (`volleys.js → reminders.js`, BY DESIGN, is the eleventh and carries its own trigger.) | §7 dependency rule | `npm run layers` — the allowlist in [check-layers.mjs](tools/check-layers.mjs) is the evidence, and it is **mechanical**: a repaid edge fails the gate as a stale pin, so unlike every row above it, **this one cannot go stale in place** |
...
1. `npm run moments` fails until the key is classified in [decide/moments.js](scripts/decide/moments.js):
   ...
   caught: the file that writes the world is pinned in `WORLD_WRITERS` ([tools/moment-writers.mjs](tools/moment-writers.mjs) since 2026-09-23 — the gate and the claim proof read the same scan) to the record its write
...
1. Declare its layer (§2) in its header comment **and in `LAYER_OF` in
   ...
   the layer is a decision made once, in writing, by whoever adds the file. A PART of a directory
   machine (`saves/`, §7) is a machine whose group is its folder (`GROUPS` in the same tool):
   only its `index.js` may be imported from outside, and the index's import list fixes the parts'
   registration order — its header says so, and the snapshot proves it.
...
4. The full live battery is about 50 minutes (29 suites, measured 2026-09-23) and the unit tests
   are about a second. Without rule 1, test time grows in lockstep with feature count forever.
5. **A change runs its own suites, not the battery (2026-09-23, user ruling).** Every live suite
   declares `export const COVERS = [...]` beside its `SECTIONS` — the machine-tier files whose
   behaviour its sections drive — and `node tools/battery.mjs --changed --list` prints what a
   change has to re-run and why (`--changed` without `--list` runs it). The map is checked
   BOTH WAYS in the verify gate (`npm run coverage`, [check-coverage-map.mjs](tools/check-coverage-map.mjs):
   every machine claimed, every claim a machine that exists, every suite in the battery) and
   proven at RUNTIME by the claim proof ([claim-proof.mjs](tools/claim-proof.mjs), printed after
   the hook coverage at the end of a battery: a claimed file is PROVEN when a record kind only it
   writes was published in that suite). ⚠ **A spine change is the full battery, honestly** — and
   so is a `decide/` file the spine imports (verdict, receipt, card and eight more), because
   `shared.js` and `auto-apply.js` reach every machine. **The full battery is the floor before a
   release**, run unattended; nothing smaller is. Adding a machine means claiming it from the
   suite that drives it — the gate refuses an unclaimed one, and there is no allowlist.

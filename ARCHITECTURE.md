...
| **D2** | `hold.js` uses **none** of the moment spine — it predates it and still runs its own clock, latch and views | §5 mandate | zero uses of `openMomentPopup`, `momentBarHTML`, `momentButton`, `armAskTimer`, `scheduleBarSync` |
| **D3** | Per-target flag writes in `hold.js`, `mastery.js` and `concentration.js` bypass the serializer while doing read-modify-write on shared per-target arrays | §4 law 2 | `queueFlagWrite` used by `saves`/`maneuvers`/`volleys` only; the topple and hold flags are mutated the unsafe way |
...
| **D5** | DECISION logic is inlined inside EDGE hook handlers almost everywhere, so almost nothing is unit-testable | §2 | 4 of 20 files are majority-pure; the rest interleave reads, judgment and writes |
| **D6** | `ui.js` ↔ `hold.js` and `mastery.js` ↔ `concentration.js` import cycles, safe only by the hoisted-function convention | §7 | works today; one non-hoisted export away from a load-order failure |
| **D7** | No lint, no formatter, no dead-code check, no package manifest, no CI — the only static check is the hook-order script | — | repo has no `package.json` |

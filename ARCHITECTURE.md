...
rows for why doing that work would make the tree worse. **D9 is PARTLY REPAID and PARTLY OPEN**, and **D10 is open and unscoped** — the two rows
still carrying open items. ⚠ They are open for opposite reasons and the difference matters: **D9 is understood and deliberately unpaid**, while **D10 is a failure class with no rule against it at all.**
...
| **D10** | ⚠ **OPEN, UNSCOPED (2026-08-23) — A HOOK NAME THE SYSTEM NEVER DISPATCHES IS INVISIBLE.** `Hooks.on("dnd5e.rollAbilityCheckV2", …)` registers cleanly, throws nothing, logs nothing, and does nothing **forever**. v1.23.0 shipped exactly that: `rollAbilityCheckV2` and `rollToolV2` do not exist — dnd5e's `Actor5e##rollD20Test` serves ability checks AND saving throws and fires only the non-V2 name, while `#rollSkillTool` fires a V2 pair and calls the tool one `rollToolCheck`. **Two of three non-attack offer paths were dead, and `smoke-d20-folds` was 12/12 green over them**, because it asserted content facts and never that a hook FIRES. ⚠ **This is the only failure class on this page with NO rule against it** — D9's `npm run layers` cannot see it (a static import-graph check never reads a string literal), and the §9 API-drift rule is about a hook CHANGING, not about one that never existed. ⚠ **The data is already extracted:** `check-hook-order.mjs` parses all 83 registrations by name, so the check is a filter over what it holds. **The hard part is sourcing the truth honestly** — dnd5e enumerates no list of the hooks it dispatches, so it must be a CURATED list checked against the system source at a pinned version: a registry in the house sense (§6), wanting its own R4-style pin so it fails when it drifts rather than silently blessing a typo. ⚠ **Interim discipline, and it is the whole of the mitigation today: a live suite must assert that a hook FIRED, never that it was registered** — `smoke-d20-folds` §4 is the worked example. **Disposition: OPEN.** The cost is one curated list plus a pin; the reason it is not built is that nobody has yet decided who owns keeping it true across a dnd5e release. | nothing — **that is the debt** (nearest: §9, the dnd5e contract) | `tools/smoke-d20-folds.mjs` §4, which asserts dispatch for the five hooks the folds depend on and is the only thing in the tree that would catch a recurrence |

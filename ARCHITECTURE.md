...
What still points at it is hold's own feature API, which is correct: `auto-damage.js` asks it
to stamp a hold, and `ui.js` draws its views. That second one is the remaining half — see D1
and D6.
...
| --- | --- | --- | --- |
| **D1** | ⚠ **HALF REPAID (2026-08-22).** The shared services left: `canAnswerFor`, `isContinuingClient`, `inRunningCombat` → `core.js` (beside `isActiveGM`/`rollerUserFor` — one §3 family); the interrupt/block list wrappers → `settings.js`. **Four machines no longer import `hold.js` at all.** What remains is `ui.js`, which draws the hold's row and popup and therefore needs `reactionItem`, `answerHold`, `continueHold` — that is the same knot as D6 and repaid the same way | §7 dependency rule | importers of `hold.js`: was `polish`, `saves`, `mastery`, `maneuvers`, `concentration`, `ui`, `auto-damage`; now **`ui` and `auto-damage`** — and auto-damage's is a legitimate feature call (`stampHoldIfInterrupted`) |
...
| **D6** | **Three** import cycles, safe only by the hoisted-function convention: `ui.js` ↔ `hold.js`, `hold.js` ↔ `auto-damage.js`, `auto-apply.js` ↔ `mastery.js`. ⚠ **D1 did NOT dissolve these, which PLAN.md expected it to** — measured 2026-08-22 | §7 | ⚠ `ui.js` ↔ `hold.js` survives because ui.js holds ~400 of its 697 lines as the hold's OWN views (the row, the popup, the reaction art, the AC read) plus two hook registrations. Breaking it means relocating those into hold.js, which moves a `renderChatMessage` and a `deleteChatMessage` registration between files and rewrites a pinned hook-order assertion — a stage of its own, not a mechanical move. ⚠ `hold.js` ↔ `auto-damage.js` is **load-bearing on purpose**: the bare `import "./auto-damage.js"` pins evaluation order and must not be "fixed". ⚠ `auto-apply.js` ↔ `mastery.js` is breakable by moving `applyDamagesWithReceipt` to its own module, but that is the damage chokepoint every machine routes through. ⚠ `mastery.js` ↔ `concentration.js` is **not** a cycle |
...
| **D8** | ⚠ **NEW, found 2026-08-23 by scoping a feature rather than by reading code.** **The post-roll fold is a KIND, but it is coded as one feature's special case.** `hitsAmong` takes `precision:` as a *named parameter* beside `held:` — so a second fold of the same shape (Heroic Inspiration's reroll, Tactical Mind's +1d10, Bardic Inspiration's die) adds a third parameter, then a fourth. **Three surveyed features are all this one kind, and the module already ships a member of it** (Precision Attack, v1.19.0). That is R4's tripwire firing: kinds arriving faster than the registry can absorb them, because the mechanism was never lifted out of the first feature that needed it | §6 R4 · §2 rule 4 | [decide/verdict.js](scripts/decide/verdict.js) `hitsAmong({targets, held, precision, roll})` — the fold channels are hard-coded arguments, not a list. ⚠ **And its precedence argument does not survive a second fold:** the doc block reasons *"the sets are disjoint, because a hold stamps hits and precision stamps misses."* A **reroll can go either way** — "you must use the new roll" can turn a hit into a miss — so precedence would have to be decided rather than inherited. ⚠ **The attack side only.** `hitsAmong` folds verdicts for attacks; the save side has no equivalent channel, so a rerolled *save* has nowhere to land today. That is the real new work, not the offer or the popup |

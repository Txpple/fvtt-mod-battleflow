...
rows for why doing that work would make the tree worse. **D9 is PARTLY REPAID and PARTLY OPEN**; **D10 is CLOSED (2026-08-23)**; **D11 is open, and it is
the one that matters.** ⚠ **Read them as a sequence, because they are one argument.** D9 is
...
a listener is on a real channel, and still cannot prove it ever hears anything. ⚠ **That is the
shape of this whole page's blind spot, said in one line: the checks are strong on SHAPE and weak
on BEHAVIOUR**, and every structural investment so far has bought more shape.

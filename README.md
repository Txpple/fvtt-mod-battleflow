...
| **The gate before the roll** | when something the module can read bends an attack roll — your own Vex on the target, a Sap on you, Prone on either side (the 5-foot rule, judged in feet whatever the scene's units), or any of the thirteen 2024 conditions — Battle Flow's popup stands in for the roll dialog: every source with its rule quoted, the net (Advantage and Disadvantage cancel, however many of each), the dialog's own choices (attack mode, ammunition, mastery, roll mode), a situational bonus, and Advantage / Normal / Disadvantage for you to press, with the net highlighted. Nothing is applied for you. Two lists switch it: Reminder Sources (the kinds) and Condition Sources (which conditions) |

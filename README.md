...
| **The gate before the roll** | when something the module can read bends an attack roll — your own Vex on the target, a Sap on you, Prone on either side (the 5-foot rule, judged in feet whatever the scene's units), any of the thirteen 2024 conditions, or a ranged attack's own range (beyond normal, beyond long, an enemy within 5 feet) — the system's own Attack Roll dialog opens, even on a shift-click, with a Battle Flow section in it: a box per source with its rule quoted, and the net (Advantage and Disadvantage cancel, however many of each), which is the highlighted button. The section follows the dialog's own dropdowns. You press; nothing is applied for you. Two lists switch it: Reminder Sources (the kinds) and Condition Sources (which conditions) |

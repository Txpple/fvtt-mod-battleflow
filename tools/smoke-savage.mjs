    ...
    if (want(1) || want(2)) {
      ...
      ok('1a. the popup asks: the tick row names Savage Attacker with "1d6 again" and "once per turn", the rule folded under; a 2 is under the average, so the die meter reads low, the row starts TICKED and "Roll again" is live (the hint, option D)',
        !!popup && /Savage Attacker/.test(textOf(popup.element)) && /1d6 again/.test(textOf(popup.element)) && /once per turn/i.test(textOf(popup.element))
          ...
          && popup.element.querySelector('button[data-action="keep"]')?.disabled === true,
        `popup=${!!popup} text="${textOf(popup?.element).slice(0, 260)}" checked=${box?.checked} again=${rollBtn?.disabled}`);
      if (box?.checked) box.click();
      const keepBtn1 = popup?.element?.querySelector('button[data-action="keep"]');
      ok('1e. both buttons, the tick picks: unticked, "Roll again" greyed and "Keep the roll" live',
        rollBtn?.disabled === true && keepBtn1?.disabled === false,
      ...
      tick(popup);
      ok('1f. ticked again, "Roll again" live and "Keep the roll" greyed', rollBtn?.disabled === false && keepBtn1?.disabled === true,
        `again=${rollBtn?.disabled} keep=${keepBtn1?.disabled}`);
    }

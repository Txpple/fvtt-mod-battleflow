...
// 2026-09-24: 30 → 31, `roll` joins the interrupt set (Slice A, ruled off prototypes/slice-a.html):
// Lucky, Warding Flare and Shadowy Dodge impose Disadvantage on an attack roll AFTER the hit showed —
// neither `ac` (the AC never moves, and a natural 20 can be undone) nor `damage` (it changes whether
// the attack hit at all). Three cost shapes are rows of INTERRUPT_ROLLS, not kinds. Savage Attacker
// (DAMAGE_EITHER) is NOT counted: one table read by one machine with no kind column — the
// CLOCK_RIDERS / DAMAGE_SHIELDS / METAMAGIC shape, and a second customer is a row.
// 2026-09-25: 31 -> 33, Lucky's Advantage half (the Halfling walk): `buy` in the reminder set (the
...
const EXPECTED_SOURCE_FILES = 101;   // 2026-09-26 (the dice that rise, group 2): 100 → 101, for scripts/decide/dice-chips.js (DECISION: a roll as the chips dice-rise.js draws, and the diceRise record a roll message carries). 2026-09-26 (the dice that rise): 98 → 100, for scripts/dice-rise.js (SPINE: the dice that rise over a token — one renderer, every rule that changes dice, the fighting styles its first customer) and scripts/hold/dice.js (a MACHINE part of the hold: a bent roll's d20s rise over the creature hit). 2026-09-26 (the fighting styles): 96 → 98, for scripts/fighting-styles.js (a MACHINE: each listed Fighting Style's face on the character, gated on what is equipped, and its number on the damage roll it fits — Great Weapon Fighting's floor, Thrown, Two-Weapon, Dueling; Defense's AC on its face) and scripts/decide/fighting-styles.js (its pure half). 2026-09-25 (the origin-feat walk, later): 95 → 96, for scripts/kit-tend.js (a MACHINE: a kit's use on a creature within reach healed from that creature's own Hit Point Dice — Healer's Battle Medic; the size picked in a popup, the die spent and the feature's own heal rolled by the elect). 2026-09-25 (the origin-feat walk): 94 → 95, for scripts/unarmed-dice.js (a MACHINE: the plain Unarmed Strike's damage swapped for the die a feature's own unarmed attack carries — Tavern Brawler — with one line on the card). 2026-09-25 (the origin feats, later): 93 → 94, for scripts/initiative-swap.js (a MACHINE: Alert's Initiative swap — asked once every combatant has an Initiative, the ally picked in a popup, the tracker written by the elect). 2026-09-25 (the origin feats): 92 → 93, for scripts/heal-rerolls.js (a MACHINE: a healing die showing a 1 rerolled at its owner's word — Healer, on a healing spell and on the feat's own Battle Medic; Empowered's popup, Savage's claim on the application). 2026-09-25 (the Orc walk): 91 → 92, for scripts/drop-to-one.js (a MACHINE: a drop to 0 Hit Points turned into a drop to 1 at dnd5e.preApplyDamage — Relentless Endurance asked, Death Ward automatic). 2026-09-25 (the Human walk): 90 → 91, for scripts/rest-grants.js (a MACHINE: what a feature gives at a rest where the platform gives nothing — Resourceful's Heroic Inspiration, riding the rest's own update). 2026-09-25 (the Halfling walk): 89 → 90, for scripts/advantage-buys.js (a MACHINE: Advantage bought with an item's use before the roll — Lucky's box on the gate, in the system's own dialog). 2026-09-25 (the Goliath walk): 86 → 89, for scripts/damage-holds.js (a MACHINE: a reduction "when you take damage" held before any damage the module applies lands — Stone's Endurance, through the applier's claim) and for scripts/rebukes.js (a MACHINE: a Reaction to taking damage aimed at the creature that dealt it — Storm's Thunder, Hellish Rebuke, Fount of Moonlight, Retaliation, Sword of Answering; Riposte's shape on the damage's landing) and scripts/decide/rebukes.js (its pure half). 2026-09-25 (the Aasimar walk): 84 → 86, for scripts/token-lights.js (a MACHINE: a use whose text sheds light carries it as the token's own light on an effect — Inner Radiance, the Light spell) and scripts/decide/token-lights.js (its pure half). 2026-09-24 (Slice A, tier 3): 81 → 84, for scripts/damage-either.js (a MACHINE: the damage dice rolled twice — Savage Attacker's popup on a weapon hit, the set rolled again, the higher standing; its own feature, neither a metamagic nor a clock rider), scripts/decide/damage-dice.js (DECISION: the damage-dice folds' patch — Empowered's per die, lifted out of metamagic.js, and Savage's per set) and scripts/decide/rescue-hit.js (DECISION: the `roll` interrupt — Disadvantage on a roll already made, its rows and its card lines). 2026-09-24: 79 → 81, for scripts/area-ask.js (a SERVICE: the ask at the area — the popup, the clock and the answer that two machines route one question through, built out of metamagic.js when its second customer had proved the shape) and scripts/decide/area-ask.js (its pure half). 2026-09-15 (the dnd5e 6.0 pass, phase 1): 77 → 79, for scripts/decide/card.js (DECISION: the card seam — what kind of card, whose, from which, off the typed message the platform writes since 6.0) and scripts/surfaces.js (CORE: the surfaces map — every platform HTML anchor in one place, gated by tools/check-surfaces.mjs). 2026-09-15 (branch effect-view): 75 → 77, for scripts/effect-view.js (a MACHINE: the effect view - buffs and debuffs on hover, on a held key, on a bar; writes nothing) and scripts/decide/effect-view.js (its pure half). 2026-09-13: 74 → 75, for scripts/decide/sequence.js (DECISION: the hit's sequence — a queued offer waits for the damage and the mastery's decision, user ruling). 2026-09-11 (later): 73 → 74, for scripts/decide/moments.js (DECISION: the moment records — every resolve the module writes, classified, read by the gate in events.js and by tools/check-moments.mjs). 2026-09-11: 72 → 73, for scripts/events.js (SPINE: the moment events — what the module publishes at a resolve, for a neighbour that keys its pictures on cards). 2026-09-09 (later): 71 → 72, for scripts/holds.js (SPINE: the hold registry — what another module asks before it plays, generalised out of metamagic.js). 2026-09-09: 69 → 71, for scripts/metamagic.js (a MACHINE: the Sorcerer's options in the cast dialog, the points spent on the card) and scripts/decide/metamagic.js (its pure half). 2026-09-05 (later): hold.js → hold/ (nine parts, the directory rule's second customer). Earlier the same day: lookup.js, decide/demand.js (the machine-tier pass, Stages 1–2); maneuvers.js → five fold files (Stage 4a); topple.js, chip-spend.js (Stage 4b); saves.js → saves/ (eight parts, Stage 4c)

...
Hooks.once("init", () => {
  ...
  game.settings.register(MODULE_ID, S.hitMenuList, {
    name: "Hit Menu",
    hint: "A choice the hit offers before the damage rolls, by the feature's own name, separated by commas — the Battle Master's on-hit maneuvers (Trip Attack, Goading Attack, Menacing Attack, Pushing Attack, Disarming Attack, Distracting Strike, Maneuvering Attack, Sweeping Attack) and the Goliath's Giant Ancestry boons (Fire's Burn, Frost's Chill, Hill's Tumble). When a listed feature stands on the attacker's sheet with a use left — a Superiority Die, or the boon's own use — the damage offer opens with its group: pick one for the hit, or none. The die is READ off the feature and rides the damage roll in the feature's own damage type (a critical hit doubles it; a Sweeping Attack's die is rolled at a second creature the card asks for), the use is spent, a maneuver's save is put to the target through the save gate, Frost's Chill's slow lasts to the start of your next turn, Hill's Tumble knocks a Large or smaller target Prone, and the card says what rode. Remove a name to keep that feature by hand.",

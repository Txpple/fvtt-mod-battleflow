...
| **Stone's Endurance**: "when you take damage" (any damage) | taken AUTOMATICALLY, never asked (user, 2026-09-25: "the rule should be stones endurance reduces automatically"): every damage the MODULE applies, an attack hit's included, is reduced before it lands while a use and the Reaction stand (`damage-holds.js`); damage applied with the card's OWN buttons or typed on a sheet is reduced by hand | only the module's own applier can make a number wait: the card's buttons call `Actor#applyDamage` straight, and `dnd5e.preApplyDamage` is synchronous — no popup can be answered inside it (`auto-apply.js` `registerDamageClaim`) | 2026-09-24; widened 2026-09-25 |

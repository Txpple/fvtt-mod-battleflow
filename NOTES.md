...
**⚠ `tools/build-release.ps1` can be refused by the machine's PowerShell execution policy** (2026-09-23: "running scripts is disabled on this system" — it ran on 2026-09-21). The policy is the user's security setting and is never overridden from here. v2.0.4 was built with Windows' own bsdtar instead — `C:/Windows/System32/tar.exe -a -c -f dist/fvtt-mod-battleflow.zip module.json LICENSE README.md <every scripts/**/*.js>` writes a zip with forward-slash entry names — and the builder's three archive checks (no backslash entry, nothing missing, every relative import resolving inside the archive) were run in Node over `tar.exe -tf`, then the archive unpacked and diffed against the tree. Same contents as the builder's, same proof.

**⚠ A HALF-AWAKE Molten box answers every WebDAV GET with a 404 page, so `deploy-house-module.mjs --check` reports every file DIFFER with ONE identical hash** (seen 2026-09-10 on the first check of the night). It is not a stopped box: a read through the MCP bridge (`get-world-info`) wakes it, and the re-check is then sane. **Never deploy on an all-identical check** — wake it, re-check, then deploy. And WebDAV never prunes: a file removed from the tree must be deleted on the box by hand, or the zip shipped instead.

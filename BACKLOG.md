...
| Item | Shape |
| --- | --- |
| **Noxious Miasma's area outliving an instantaneous breath** (2026-09-10 — the user's report on the Adult Green Dragon, beside Poison Breath) | ⚠ **THE SECOND CUSTOMER of the row below has landed.** Poison Breath was a plain bug (a feature's demand read the ITEM's duration, and a `feat` has none — fixed the same day, `saves/demand.js`, smoke-saves §8f). Noxious Miasma is the Hypnotic Pattern class: the Monster Manual writes its activity duration as `1 turn` — the −2 AC penalty's clock, not the cloud's — so by the sweep's three buckets its area is a non-concentration duration area, the GM's to clear, exactly like Grease. No data rule separates the two; only the text does. **The user's word decides the shape:** leave it the GM's, or build the fourth bucket the row below names — a *Spent Areas* list (the Block List's shape: names whose area is spent at the last verdict whatever the data says), Noxious Miasma and Hypnotic Pattern its first two rows. Not built; waiting on the ruling. |

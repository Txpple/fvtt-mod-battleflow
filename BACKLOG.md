...
| **"The ring is stuck in place"** | **Not reproduced.** On the Lair a PLAYER's animated drag moved Invictus's ring with him three times, and a ring re-raised mid-walk landed on the destination. Hardened anyway: rings are raised from the token's committed position, and the sweep re-bases a drifted ring after each move of its source (smoke-emanations §4d). If it recurs: note the scene, whether it is the ACTIVE one, and whether the ring moves at all. |

...
> ⚠ **These four are the one exception to the charter at the top of this file:** they are live
...
| --- | --- |
| **Careful Spell did nothing — two Fireballs rolled the excluded allies' Dex saves and applied their damage** (four reverts the first time, five the second; the second revert is what un-killed Gren, who went to 0 from his own spell through a timer-rolled save) | **Known:** the module has no metamagic at all (`grep -i metamagic scripts/` is empty; SWEEP §2 counts it under *options nothing grants*), so the save prompt asks everyone under the template and the ask timer (`saves/ask.js`, `fireSaveTimer`) rolls for whoever has not answered — exactly what it was built to do, aimed at the wrong people. **Two honest shapes:** (a) the narrow one — Careful Spell as a pre-cast pick that removes the chosen creatures from the save prompt's list before it posts (the prompt already carries its own target list; a filter on it is small); (b) the one the user named — **metamagic as a class**, on the pattern Sneak Attack and the maneuvers set: a checkbox/choice on the gate, backed by sorcery points as the pool (`superiority-uses.js` is the spend shape), each option a registry row (Careful → excluded from the save; Twinned → a second target; Quickened → bonus-action cast; Heightened → disadvantage on the save; Empowered, Extended, Distant, Subtle, Seeking, Transmuted as their own rows). (b) is the long-term shape and the house pick when costs are stated (memory rule); (a) is the evening cut. **Also on the table, independent of metamagic:** *should the timer roll a save for a PC at all?* Today the buzzer rolls for everyone who did not answer; a PC-only hold-the-line (buzzer fires, NPCs roll, PCs stay pending) is one branch in `fireSaveTimer`. **Settles it:** a Sorcerer fixture with Careful Spell, a Fireball over two allies, `smoke-saves` asserting no ask posts for the excluded two. |
| **Spirit Guardians rolled a save for Gren out of range** (flagged live and reverted) | **Known:** the aura is a Region built by the platform (`RegionDocument.createTokenEmanation`, `emanations.js` header), and the region's events are the fast path with the floor sweep as the truth. **Guessed, three ways:** the Region's radius is the class's range plus half the token's base (emanations.js:532) — right for a Large caster, but check what it produces when the ACTIVITY's size overrides (`activitySizeOf`); walls — a Region does not know about walls at all, so a token behind a wall but inside the circle IS inside the Region, and the rules say an emanation is blocked by total cover (unmodelled today, deliberately: no line-of-sight test anywhere in emanations.js); or the template Gren was measured against was not the aura's (user's word: *possible mismatch on template*) — a stale spent-area from an earlier cast under the same caster. **Settles it:** the sandbox scene, Gren's token at the distance the table saw, a probe that prints the Region's polygon and Gren's centre (`probe-emanations.mjs` is the start). If it is the wall, the fix is a cover test on tokenEnter, and it wants its own ruling (DESIGN §6) before code. |
| **A stale Shield effect survived a revert, so the next hit offered no reaction** (that damage was reverted by hand too) | **Known:** the hold's answer path applies Shield as an ActiveEffect with the platform's own clock (`hold/answer.js`); the module's reverts are per-receipt (`decide/receipt.js`) and take back HP and the chips the receipt names. **Guessed:** the rewind undid the DAMAGE receipt but not the answer's effect, because the effect is the platform's (the cast's own +5 with its `until start of next turn` clock), not a chip the receipt lists — and a Shield effect still standing by name is exactly what the hold lookup (`hold/lookup.js:224`) reads as *already answered*, so the offer stands aside. Two shapes: the receipt learns the effects the answer created (so a revert of the hold's damage also removes its Shield), or the offer stops reading a leftover effect as an answer and reads the hold's own flag instead. **Settles it:** `smoke-hold` — answer a hold with Shield, revert the hit, hit again, assert the offer posts. **More from the table (user, 2026-09-09):** the leftover showed on Gren's sheet under the SYSTEM's *Unavailable Effects* panel (dnd5e's own section for effects whose source item is not active — unequipped, unattuned, or gone), named **Immutable Barrier**, beside the four attunement-waiting magic items; it was removed by hand. That places the effect in the SUPPRESSED state, and the hold lookup (`hasReactionEffect`, hold/lookup.js) tests `!e.disabled` and matches by NAME or origin but never `isSuppressed` — a suppressed leftover counts as *standing* there, while the gate's effect read (reminders.js:214) already excludes suppressed effects. Two facts to check when picked up: whether Immutable Barrier is the name the cast's clone wrote (the answer path applies from an item CLONE, so origin uuids differ — the header above the lookup says so) and why its source went unavailable (a clone's origin resolving to nothing fits). By 2026-09-09 the effect is no longer on the sandbox copy of Gren; the sandbox is a prod copy and may predate the session, so reproduce, do not look for it. |
| **Private rolls got toggled on by accident** (user: *need a click box, maybe in Combat Plus instead*) | **Known:** the only roll-mode the module sets is concentration's (`concVisibility`, `concentration.js`); the private-roll toggle the table hit is the platform's chat-bar roll-mode dropdown (or `dnd5e`'s own setting), not a Battle Flow control. **User's lean:** a one-click visible toggle, and Combat Plus is the likelier home (it owns tracker-side chrome; Battle Flow never takes platform patches — memory rule, the Misty Step precedent went to miscpatches). Nothing to do here beyond the pointer; if it lands, it lands in a sister repo. |

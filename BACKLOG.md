...
| **`smoke-shields` flakes in a FULL run and passes alone** (2026-09-05, three full runs: §3b twice — a RANGED hit from 5 ft posts a shield card — and §4d once — the Death Armor chit does not die with the turn) | Every section passes when run by itself (`--section 3`, `--section 4`), so the code under test is not wrong; something an EARLIER section leaves behind is. The run's own console carries two leads: `Cannot read properties of null (reading 'position')` and `ActiveEffect "…" does not exist!` — the second is a double delete of a chit or ward racing a sweep, the first a token read after its scene left. Nothing this day's commits touch (damage-shields.js is unchanged since the overnight run). **Next step:** run the suite with `--section 1,2,3` and `--section 2,3` to find the section whose residue makes §3b strike, and read the two stacks with `e.stack`. Not owed by any commission; parked until a full battery is next wanted green without a rerun. |

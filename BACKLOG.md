...
| **A hit within 5 feet of a Paralyzed or Unconscious creature is a Critical Hit** (user, 2026-09-02 — TODO) | The glossary clause the gate already quotes in its Paralyzed and Unconscious boxes — *"Any attack roll that hits you is a Critical Hit if the attacker is within 5 feet of you"* — is shown and not honoured: the hit lands as a plain hit and the damage rolls unmultiplied. This is an OUTCOME, not a decision (R1 says automate it): on a hit, when the target carries either status and the attacker's token is within 5 feet (the distance Prone already measures), the damage roll is critical. The shape to measure: where dnd5e decides a damage roll is critical for an attack that did not roll a 20 (`rollDamage`'s `critical` config / `isCritical` off the attack message) and whether a pre-roll-damage hook can promote it; the crit badge and the damage tray must agree. Both statuses, not one — the example is the class. |

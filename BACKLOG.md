...
| **A pass over the pack for target effects flagged as the wielder's passive** (2026-09-04, user: "you should probably do a universal pass on that") | Goading Attack's Goaded ships `transfer: true` — a passive on the wielder — and the walk lost a whole afternoon to it (NOTES §2). The module now corrects a HIT-OPTION row's target-facing effects on the wielder's copy and presses a lost one from the compendium. What is NOT done is the corpus: which other 2024 save/damage activities link an effect the pack flagged `transfer: true`? That is one filter over `scan-corpus.mjs`'s JSON (`activities[].effects` × `effects[].transfer`) — a ten-minute live scan with the user out of the world — and then a row per hit, wherever the machine that applies it lives (the saves machine's own application skips nothing on the flag; the sweep's item 1 save-side bends are the likely home). **What would un-park it:** the next feature that applies a pack effect through a maneuver-shaped row, or a table report of a save that "did nothing". |

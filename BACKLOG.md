...
| **The hit menu takes ONE pick per hit, across its groups** (2026-09-24, Slice A) | The pick is one record (`hitPick` on the attack, `hitManeuver` on the damage), so since Giant Ancestry joined Combat Superiority the offer's wire keeps one tick on the whole menu and `decide/hit-menu.js` `hitPick` drops a two-group pick — a Goliath Battle Master cannot ride Fire's Burn and a maneuver on one hit, which the rules allow. The array shape (`hitPick`/`hitManeuver` as a list: one rider part, one spend and one card line per pick; the moment published per pick) is the fix. **Un-parked by** a Goliath Battle Master sitting down at the table. |

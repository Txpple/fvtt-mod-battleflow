...
1. **The modal sequence.** A workflow where several windows are answered **in order**, and the
   ...
   ⚠ **Not this (2026-09-13): the hit's own sequence** — the bash offer queued behind the damage
   and the mastery's decision (law 7's second bullet, `decide/sequence.js`). That one is decided
   from ONE hit's records at the damage chokepoint, no window waits on another window; it
   neither builds nor pre-empts the modal sequence, which stays parked on the user's call.
2. ~~**The pictures that never play.**~~ ✅ **BUILT 2026-09-11 on the user's word ("go"): the moment events** — [events.js](scripts/events.js) `publishMoment`, the hooks `battleflow.moment` / `battleflow.<event>`, a plain payload, three publishers (the hit menu's die, Sneak Attack, a hold answered by a cast; Parry under both words), ARCHITECTURE §7 *The moment events*. The user's ruling on the shape: *"from the battleflow perspective, no real dependency because fx studio is optional. events would just fail silently"* — Battle Flow fires a hook it does not know anyone hears; FX Studio grows a reader in ITS repo (its commission, written into its PLAN.md the same day). **Later the same day, VERSION 2 — THE GATE** (the user: *"Dread Ambusher did not, though if I use the card it does … something architecturally solid so as this grows, it's not missed / no drift"*): the three hand-placed publishers became three rows of a REGISTRY over the module's own records ([decide/moments.js](scripts/decide/moments.js), 32 resolve rows, 17 words), ONE publisher in events.js watching message creates and updates, and [tools/check-moments.mjs](tools/check-moments.mjs) failing the build on any written key that is not classified — every resolve publishes (riders, folds, masteries, shields, spends, receipts, saves, breaks included), and what plays is FX Studio's manager's call. ARCHITECTURE §7. The survey that led here: An ability consumed through a Battle Flow popup posts no
...
## From play — Session 8 (2026-09-22), what the fix pass of 2026-09-23 left for a ruling

The pass fixed Hunter's Mark (the pack's lineage in `system.origin.item`), Shield Bash's reach,
the moment popups taking the keyboard (Tactical Mind's accidental Pass), and the closed demand
that kept a Fireball circle standing (NOTES §1, §2, §4 carry the measurements). These stand:

| Report | What is known, and what would settle it |
| --- | --- |
| ~~**The aura on a scene nobody ACTIVATED**~~ — "allies next to Invictus not getting the +2" | ✅ **RULED AND BUILT 2026-09-23** (the proposal below, user: *"ok sounds good"*; DESIGN §6 *Emanations*, amended): an emanation stands on every LIVE scene — the active one and every scene a connected player views (a GM's view counts only while no player is connected) — with one member copy per aura (`liveScenes`, `emanationGroup`, `groupMembers` in decide/emanations.js), pinned by smoke-emanations §11g–l and smoke-twoclient `pull` (a real player client on an unactivated scene). The measurement that led here: Feature rings stand on the ACTIVE scene only (user, 2026-09-04: a ring on each of 22 scenes stacked the effect seventeen times). Session 8 played on scenes that were never activated: Jetten spoke from *The Long Rest – Upper Floor* at 7:43 while the ring went up on the Ground Floor (the active scene) at 7:51; the party was in the *Apothecary* at 8:16 with no ring anywhere near it. The shape that would answer it: a ring on the active scene AND on every scene a connected user is VIEWING, with one member effect per (source, aura) on an ally actor, so two scenes can never stack — the user's call first (DESIGN §6). |
| **A TRAY-applied copy still reads the pack as its source to the PLATFORM** | This module's reader copes (it tries every origin field); the platform's `getSourceActor` does not, so a `sourceStart`/`sourceEnd` clock on an effect applied from the card's own tray button expires at the wrong turn edge. The platform's gap: a Misc Patches patch (rewrite `system.origin.item` from the activity at `preCreateActiveEffect`), or a one-time data sweep clearing the stale pack `item` from the 207 world templates. Neither built; either is small. |
| **Tactical Mind used from the SHEET after the offer passed** | Native: the card spends the Second Wind use and shows a "Bonus" roll button; the d10 is not composed into the failed check. The rescue offer does compose it — the table's miss was the accidental Pass (fixed). Composing a sheet use onto the actor's last failed check is buildable (the ARMED pattern, d20-folds.js) and wants a ruling on the window it looks back over. |
...
| **An ability that lands as an effect shows no chit on the token** (user observation, 2026-09-03 — parked, undecided) | Steady Aim is the example and the class is every `USE_CHIPS` row and every feature whose use puts an ActiveEffect on the sheet: the effect exists, the gate reads it, the roll spends it — but the TOKEN shows nothing, because the chip carries no status and Foundry paints a token icon only for effects that do. The mastery chips (Vexed, Sapped, Slowed) have the same shape. User: *"not sure if it's a good thing or bad thing because stuff would stack up too much, but something to think about."* The two honest answers: give the module's chips a status/icon so the token says what the sheet says (a data change on the chip row, one line each, and the platform's expiry keeps them tidy), or leave the token clean and let the gate's box be the reminder (today's shape). **A third, the user's (same day): a BUFF BAR** — prototyped and parked 2026-09-03 ([prototypes/buff-bar.html](prototypes/buff-bar.html)); ⚠ **RETIRED 2026-09-15 with the chit layer it was the first slice of** (user: every placement was "an obtrusive UI"; *"we just need to show debuffs and buffs. not avail actions"*). **What replaced it: THE EFFECT VIEW — ✅ BUILT AND MERGED 2026-09-15** (DESIGN §6 carries the shape and every ruling; `scripts/effect-view.js`, `scripts/decide/effect-view.js`, `tools/probe-effect-view.mjs` 16/16): the bar above the hotbar, the hover card, the held Alt; buffs and debuffs only; the bar the one interactive surface (the name opens the sheet's full list, a chip's fold carries its one write for an owner). The four questions were ruled on the live sandbox. The prototype that led here stays as the record: [prototypes/effect-views.html](prototypes/effect-views.html) (also at https://claude.ai/code/artifact/acdf710b-a0cd-406d-af2a-a204175f8eaa). Nothing open here. |
| **The abilities sweep** (surveyed 2026-09-02; **SHELVED 2026-09-03**, user: "a longer term project") | SWEEP.md, a deliberate fifth document for the sweep's length: the 2024 corpus sorted into the walk's eight mechanism families, what each kind (race → class → subclass → feat → spell) would need, and a suggested order. **Its three questions are ruled** (SWEEP §5: ignore 2014; ONE hit-menu popup per hit grouped by feature/class, smites excluded as a separate Bonus Action; ONE shared Effect Sources list for both gates) and the corpus was rescanned the same day with the numbers holding. **Start at SWEEP §0 when it is picked up** — it says where to begin (item 1, the save-side bends) and what needs a prototype first (item 2). Nothing scheduled. ⚠ **The corpus grew 2026-09-24** (the row below): the rescan is the sweep's first step now, before item 1. |
| **Arcana Unleashed — a fifth premium book, installed on both boxes and unread by the module** (2026-09-24, user: *"we now have Arcana Unleashed … a core rule expansion"*) | A 2024-rules expansion on magic in the Tasha's shape (`dnd-arcana-unleashed` v1.0.1): 8 subclasses and 61 features, 37 feats, 33 spells, 69 magic items with a standalone Active Effects pack, a 38-monster bestiary. Measured on the sandbox by [tools/probe-premium-module.mjs](tools/probe-premium-module.mjs) — pack indexes only; SWEEP §2 carries the counts and the names worth reading first. **Nothing the module does today changes:** zero of its names collide with a registry key, a feature field or a settings default, and none of the 33 spells shares a name with a PHB spell, so no name-keyed row fires on it and no per-spell list (Twinned's exceptions, Spent Areas, Chosen Areas, Effect Choices, Damage Saves, the volleys) has read it. **What would settle it:** the corpus rescan (`scan-corpus.mjs` reads every Item pack; `classify-corpus.mjs` ranks its four Item packs since 2026-09-24), then the per-spell lists read against its 33 spells, then the rows — it is a slice of the sweep, not a pass of its own. ⚠ Its `.effects` pack is the first standalone ActiveEffect compendium in the house (38 enchantments, 2 base effects — one is *Dodging*, carrying the `dodging` status the save gate already reads); the effect readers match effects ON ACTORS by name and have never met a compendium effect, so a feature that applies one from the pack wants a measurement before a row. |

...
| **smoke-hold §8 has not run live** (2026-09-02) | The section (a text-only Uncanny Dodge on Gren: found by name, held as a damage kind, the cast answer resolves it and the receipt row reads ×0.5) was written and unit-backed while the user was connected as GM, which the harness refuses. Run `node tools/smoke-hold.mjs --section 8,1,3` first thing; the whole battery after. |
| **The corpus scan's activation column** (2026-09-02) | `tools/scan-corpus.mjs` first read an activity's activation only when overridden — wrong for features, right for spells (NOTES §2). Fixed in the scanner, not re-run (same reason as above). Re-run into the scratchpad and re-classify before the sweep is scoped; SWEEP.md's "reaction" and "interrupt" columns were judged from text meanwhile. |
| **The abilities sweep** (2026-09-02 — survey only) | SWEEP.md, a deliberate fifth document for the sweep's length: the 2024 corpus sorted into the walk's eight mechanism families, what each kind (race → class → subclass → feat → spell) would need, and a suggested order. Three questions for the user sit at its end. Nothing scheduled. |
| **The reminder gate on ability checks** (the save half DELIVERED 2026-09-02 — DESIGN §5 *the save gate*) | The gate now reads attack rolls and saving throws. Ability checks have their own bends (Poisoned and Frightened on checks) and the same templated pre-roll hook (`preRollAbilityCheckV2`). Same mechanism, a third table. **Not asked for; surveyed only.** |
| **The save gate's two house popups left standing** (2026-09-02) | Option E retired the SAVE DEMAND's popup into the system's dialog; the Topple demand (mastery.js `showTopplePopup`) and the concentration ask (concentration.js) are still house popups standing in for the same dialog. Both roll a save the gate would read (a Paralyzed creature's Topple save cannot succeed either). The shape is the demand's: `rollSavingThrow` with `configure: true` and a `DialogCarried` on the options, adopted into `livePopups`. **Not asked for; the commissioned surface was the demand.** |
...
| **A `SAVE_PRESSES` row is one spell; the pack has more like it** (2026-09-02) | Web is the row (the 2024 PHB ships it with no Restrained effect). Every other bare save-with-a-condition in the premium packs is a row away — Entangle, Hold Person's Paralyzed if it ships bare — and the survey is `tools/probe-web.mjs`'s shape over each. **Not asked for beyond Web.** |
